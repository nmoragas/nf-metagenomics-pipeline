...
process BOWTIE2_HUMAN_REMOVAL {
    ...
    output:
    tuple val(sample_id), path("${sample_id}_nohuman_R1.fastq.gz"),
                          path("${sample_id}_nohuman_R2.fastq.gz"), emit: reads
    ...
    script:
        ...
        --un-conc-gz ${sample_id}_nohuman_R%.fastq.gz \\
        -S /dev/null \\
        2> ${sample_id}_bowtie2_human.log
        """
    stub:
    """
    touch ${sample_id}_nohuman_R1.fastq.gz ${sample_id}_nohuman_R2.fastq.gz
